const User = require('../app/db/models/users.model')
const router = require('express').Router()
const responseCreator = require('../app/helpers/response')
const sendActivationEmail= require('../app/helpers/sendEmail')
...
router.get('/allUsers',auth,async(req,res)=>{
    let CurrentPage=  parseInt( req.header('CurrentPage'))
    let dataPerPage= parseInt(req.header('dataPerPage') )
    res.status(200).send({
        apiStatus: true,
        
        date: await User.find({},req.body.fields,{ skip: CurrentPage*dataPerPage, limit: dataPerPage }),
        message: "data featched"
    })
})
